...
# Install Go: download and extract if not already present
install_setup_go() {
  ...
  if [[ -x "${go_install_dir}/bin/go" ]]; then
    echo "::notice::Go ${version} already installed at ${go_install_dir}"
    configure_setup_go "${go_install_dir}" "${gopath}" "${version}"
    return 0
  fi
