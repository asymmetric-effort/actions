#!/usr/bin/env bash
# Tests for actions/gh-release/scripts/upload-assets.sh
...
source "${ACTION_DIR}/scripts/upload-assets.sh"
...
assert_eq "2" "${#_RESOLVED_FILES[@]}" "matches two files from two patterns"
...
assert_eq "3" "${#_RESOLVED_FILES[@]}" "matches three files with wildcard"
...
resolve_files "dist/*" "${tmp_dir}" "false"
# Should only match files, not subdir
has_dir=0
i=0
...
# shellcheck disable=SC2034  # Used by deduplicate_files via nameref
